...
public class CompletableFuturexamples {

    private void iWork() throws InterruptedException {
        int count = 0;
        while (count < 10) {
    ...
    public void thenRunExample() throws Exception {
        CompletableFuture<Void> gtt = goToTrash();
        gtt.thenRun(() -> {
            int count = 0;
            while (count < 3) {
    ...
    public void thenAcceptExample() throws Exception {
        CompletableFuture<String> bm = buyProduct("Молоко");
    ...
    public static void main(String[] args) throws Exception {
        CompletableFuturexamples exampCF = new CompletableFuturexamples();
       /* exampCF.iWork();
        CompletableFuture<String> product = exampCF.buyProduct("Bred");
        System.out.println(product);
        exampCF.goToTrash();
        exampCF.runAsyncExample();
        exampCF.supplyAsyncExample();
        exampCF.thenRunExample();
        exampCF.thenAcceptExample();
        exampCF.thenApplyExample();
        exampCF.thenComposeExample();
        exampCF.thenCombineExample();
        exampCF.allOfExample();*/
       exampCF.anyOfExample();
    }
}